...
export default function EnvVarsTab({ project, envVariables }: { project: ProjectI, envVariables?: Record<string, string> }) {
...
const envVars = Object.keys(envVariables ?? {}).filter(key => key !== "").map((key) => {
    return {
      key: key?.toUpperCase(),
  ...
  return (
    <div className="space-y-8">
      ...
      {
        envVars?.length === 0 ? <div className="">
          <EmptyComponent
            icon={<Lock size={40} className="text-white/30" />}
            description="No environment variables"
            caption="You can't delete or edit environment variables after creation for now"
          />
        </div> : <div className="bg-[#0a0a0a] border border-white/10 rounded-xl overflow-hidden">
