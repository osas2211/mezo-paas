  ...
  return (
    <div>
      <div className="border border-white/10 bg-white/5 p-1">
        <div className={`border border-white/20 bg-dark p-6 min-h-50 space-y-5 ${props.twHeight}`}>
          <div className="text-xl font-semibold font-sans">
            <h3 className="text-[16px] font-medium">{props.title}</h3>
            {props.children || (
              <>
                <div
                  className={`flex flex-col gap-1.5 justify-center items-center font-normal ${props.action ? "md:h-35" : "md:h-50"
                    }`}
                >
