...
export class BlockchainListenerService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        this.provider = new ethers.JsonRpcProvider(this.config.get<string>('MEZO_RPC_URL') || "https://rpc.test.mezo.org");
        ...
        try {
            ...
            this.logger.log(`🎧 Starting manual event polling from block ${this.lastPolledBlock}...`);
    ...
    private async handleAccountToppedUp(userWallet: string, amountWei: any) {
        try {
            this.logger.log(`[DEBUG] Raw AccountToppedUp: ${userWallet}, ${amountWei}`);
            ...
            this.logger.error(`Error processing AccountToppedUp: ${error.message}`, error.stack);
        }
    }

    private async handleCollateralLocked(userAppWallet: string, amountWei: any, unlockTime: any) {
        try {
            this.logger.log(`[DEBUG] Raw CollateralLocked: ${userAppWallet}, ${amountWei}, ${unlockTime}`);
            ...
            this.logger.log(`🐳 Whale ${userAppWallet} locked ${amountLocked} BTC. Granting ${stakedCredits} Staked Credits.`);

            const wallet = await this.prisma.wallet.findUnique({ where: { address: userAppWallet }, include: { user: true } });
            ...
            this.logger.error(`Error processing CollateralLocked: ${error.message}`, error.stack);
        }
    }

    private async handleCollateralWithdrawn(whaleWallet: string, amountWei: any, wasSlashed: any) {
        try {
            this.logger.log(`[DEBUG] Raw CollateralWithdrawn: ${whaleWallet}, ${amountWei}, ${wasSlashed}`);
            this.logger.warn(`📉 Whale ${whaleWallet} withdrew collateral. Revoking Staked Credits!`);

            const wallet = await this.prisma.wallet.findUnique({ where: { address: whaleWallet }, include: { user: true } });
            ...
            this.logger.error(`Error processing CollateralWithdrawn: ${error.message}`, error.stack);
        }
    }
}