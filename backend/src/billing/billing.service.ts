...
export class BillingMeterService {
  ...
  async meterUsage() {
    ...
    for (const user of activeUsers) {
      ...
      const totalDailyCost = user.projects.reduce((sum, p) => sum + Number(p.dailyCreditCost), 0)
      ...
      if (Number(user.wallet?.creditBalance) >= totalDailyCost) {
        await this.prisma.user.update({
          where: { id: user.id },
          data: { wallet: { update: { creditBalance: String(Number(user.wallet?.creditBalance) - totalDailyCost) } } }
        })
      } else {
        // THEY RAN OUT OF MONEY. WE STOP SERVERS.
        this.logger.warn(`User ${user.wallet?.address} ran out of credits. Shutting down apps...`)
        ...
        for (const project of user.projects) {
          ...
          await this.prisma.project.update({ where: { id: project.id }, data: { active: false, deployment: { update: { status: DeploymentStatus.SUSPENDED } } } })
